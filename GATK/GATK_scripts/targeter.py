...
for name in sampledirnames:
    ...
    file = open (name + "targetcreator.sh", "w")
    file.write("#!/bin/bash\n")
    ...
    file.write("-T RealignerTargetCreator \\\n")
    ...
    file.write("-o " + root_path + name + "/" + name + ".rmdup.sorted.bam.list") 
    file.close()
    args=['sbatch', name + "targetcreator.sh"]
