...
bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Dplex_uniq_sfs", rep = 130)


bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/bootstraps/Dplex_eu_data_file", rep = 100)
bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Dplex_ap_sfs", rep = 130)
bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Dplex_shared_sfs", rep = 130)
...
bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Msexta_uniq_sfs", rep = 130)

bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Msexta_eu_sfs", rep = 130)
bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Msexta_ap_sfs", rep = 130)
bootstrapData(inputfile="/Users/Andrew/Documents/polyDFE/NewBoots/Msexta_shared_sfs", rep = 130)
...
#let's do dimorphic sperm
setwd("/Users/Andrew/Documents/polyDFE/bsData/Dp_eu/")
dpeu<- lapply(Sys.glob("Dplex_eu_out_strap*"), parseOutput)
setwd("/Users/Andrew/Documents/polyDFE/bsData/Dp_ap/")
dpap<- lapply(Sys.glob("Dplex_ap_out_strap*"), parseOutput)
setwd("/Users/Andrew/Documents/polyDFE/bsData/Dp_sh/")
dpsh<- lapply(Sys.glob("Dplex_shared_out_strap*"), parseOutput)
...
dpeua<-unlist(modify_depth(dpeu,2,"alpha"))
...
dpeuasem<-sd(dpeua)/sqrt(length(dpeua))
dpeuacis<-c(mean(dpeua)-2*dpeuasem,mean(dpeua)+2*dpeuasem)
...
dpeutops<-length(dpeu)
dpeudfemat<-data.frame(matrix(NA, nrow = dpeutops, ncol = 7))
for(i in 1:dpeutops)
{
dpeudfemat[i,]<-unlist(lapply(dpeu[[i]],getDiscretizedDFE))
}

dpeubsest<-colMeans(dpeudfemat)

names(dpeubsest)<-c("<-100","(-100,-10)","(-10,-1)","(-1,0)","(0,1)","(1,10)","10<")

dpeusdsdfe<-apply(dpeudfemat, 2, sd)
dpeusemdfe<-dpeusdsdfe/sqrt(dpeutops)

dumx<-seq(0.5,7.5,1)
dpeuciHi<-dpeubsest+2*dpeusemdfe
dpeuciLo<-dpeubsest-2*dpeusemdfe
...
gap.barplot(msbgbsest,xlim=c(0.5,5.4),main="Carolina sphinx moth",gap=c(from,to),col=effcols,ytics=seq(0,1,0.1),
xaxlab=rep("",7),xlab="",ylim=c(0,0.44),ylab="Proportion",xaxt="n",cex.lab=1.5,las=1,cex.axis=1.4,cex.main=1.8)
...
text(3,0.35,"Background Genome", cex=1.5, font=2)

# bottom, left, top, right margins
par(mai=c(0.06,0.14,0.42,0.06))
gap.barplot(dpbgbsest,xlim=c(0.5,5.4),main="Monarch butterfly",gap=c(from,to),col=effcols,ytics=seq(0,1,0.1),
xaxlab=names(msbgbsest),xlab="",ylim=c(0,0.44),ylab="",xaxt="n",las=1,yaxt="n",cex.main=1.8)
...
# bottom, left, top, right margins
par(mai=c(0.06,0.64,0.02,0.06))
gap.barplot(mswsbsest,xlim=c(0.5,5.4),main="",gap=c(from,to),col=effcols,ytics=seq(0,1,0.1),
xaxlab=names(mswsbsest),xlab="",ylim=c(0,0.44),ylab="Proportion",las=1,xaxt="n",cex.lab=1.5,cex.axis=1.4)
...
text(3,0.35,"Whole Sperm
Proteome", cex=1.5, font=2)

# bottom, left, top, right margins
par(mai=c(0.06,0.14,0.02,0.06))
gap.barplot(dpwsbsest,xlim=c(0.5,5.4),main="",gap=c(from,to),col=effcols,ytics=seq(0,1,0.05),
xaxlab=names(mswsbsest),xlab="",ylim=c(0,0.44),ylab="",xaxt="n",las=1,yaxt="n")
...
# bottom, left, top, right margins
par(mai=c(0.66,0.64,0.02,0.06))
gap.barplot(msorbsest,xlim=c(0.5,5.4),main="",gap=c(from,to),col=effcols,ytics=seq(0,1,0.1),
xaxlab=names(msorbsest),xlab="Effect Class",ylim=c(0,0.44),ylab="Proportion",las=1,cex.lab=1.5,cex.axis=1.4)
...
text(3,0.35,"Sperm Homologs", cex=1.5, font=2)

# bottom, left, top, right margins
par(mai=c(0.66,0.14,0.02,0.06))
gap.barplot(dporbsest,xlim=c(0.5,5.4),main="",gap=c(from,to),col=effcols,ytics=seq(0,1,0.05),
xaxlab=names(dporbsest),xlab="Effect Class",ylim=c(0,0.44),ylab="",las=1,yaxt="n",cex.axis=1.4,cex.lab=1.5)
